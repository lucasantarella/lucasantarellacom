...
function $h(a) {
    return ("0" + Math.max(0, Math.min(255, Math.round(a))).toString(16)).slice(-2)
}
function _i(a, b) {
    $t("div")[a].innerHTML += b
}
function _h(a) {
    return hires ? Math.round(a / 2) : a
}
...
function Init() {
    for (var b = 0; b < n; b++)star[b] = new Array(5), star[b][0] = Math.random() * w * 2 - 2 * x, star[b][1] = Math.random() * h * 2 - 2 * y, star[b][2] = Math.round(Math.random() * z), star[b][3] = 0, star[b][4] = 0;
    var c = $i("StarField");
    c.style.position = "absolute", c.width = w, c.height = h, context = c.getContext("2d"), context.fillStyle = "rgb(0,0,0)", context.fillStyle = "rgba(0,0,0," + opacity + ")", context.strokeStyle = "rgb(255,255,255)"
}
function Anim() {
    mouse_x = cursor_x - x, mouse_y = cursor_y - y, context.clearRect(0, 0, w, h);
    for (var a = 0; a < n; a++)test = !0, star_x_save = star[a][3], star_y_save = star[a][4], star[a][0] += mouse_x >> 4, star[a][0] > x << 1 && (star[a][0] -= w << 1, test = !1), star[a][0] < -x << 1 && (star[a][0] += w << 1, test = !1), star[a][1] += mouse_y >> 4, star[a][1] > y << 1 && (star[a][1] -= h << 1, test = !1), star[a][1] < -y << 1 && (star[a][1] += h << 1, test = !1), star[a][2] -= star_speed, star[a][2] > z && (star[a][2] -= z, test = !1), star[a][2] < 0 && (star[a][2] += z, test = !1), star[a][3] = x + star[a][0] / star[a][2] * star_ratio, star[a][4] = y + star[a][1] / star[a][2] * star_ratio, star_x_save > 0 && star_x_save < w && star_y_save > 0 && star_y_save < h && test && (context.lineWidth = 2 * (1 - star_color_ratio * star[a][2]), context.beginPath(), context.moveTo(star_x_save, star_y_save), context.lineTo(star[a][3], star[a][4]), context.stroke(), context.closePath());
    timeout = setTimeout("Anim()", FPS)
}
function move(a) {
    a = a || event, cursor_x = a.pageX - canvas_x, cursor_y = a.pageY - canvas_y
}
function KeyManager(a) {
    switch (a = a || event, key = a.which || a.keyCode) {
        case 27:
            flag = !flag, flag ? timeout = setTimeout("Anim()", FPS) : clearTimeout(timeout);
            break;
        case 32:
            star_speed_save = 0 != star_speed ? star_speed : star_speed_save, star_speed = 0 != star_speed ? 0 : star_speed_save;
            break;
        case 13:
...
function MouseWheel(a) {
    a = a || event;
    var b = 0;
    a.wheelDelta ? b = a.wheelDelta / 120 : a.detail && (b = -a.detail / 3), star_speed += b >= 0 ? -.2 : .2, a.preventDefault && a.preventDefault()
}
function Start() {
    Resize(), Anim()
}
function Resize() {
    w = GetScreenSize()[0], h = GetScreenSize()[1], x = Math.round(w / 2), y = Math.round(h / 2), z = (w + h) / 2, star_color_ratio = 1 / z, cursor_x = x, cursor_y = y, Init()
}
function Inverse(a) {
...
$(document).ready(function () {
    ...
    function b(b) {
        ...
        var h, g = this, i = {}, j = b ? a.fn : a, k = arguments, l = 4, m = k[1], n = k[2], o = k[3];
    ...
    var c = {}, d = "doTimeout", e = Array.prototype.slice;
    a[d] = function () {
        return b.apply(window, [0].concat(e.call(arguments)))
    }, a.fn[d] = function () {
        var a = e.call(arguments), c = b.apply(this, [d + a[0]].concat(a));
        return "number" == typeof a[0] || "number" == typeof a[1] ? this : c
    }
}(jQuery);
var URL = document.location.href, flag = !0, test = !0, n = parseInt(URL.indexOf("n=") != -1 ? URL.substring(URL.indexOf("n=") + 2, URL.substring(URL.indexOf("n=") + 2, URL.length).indexOf("&") != -1 ? URL.indexOf("n=") + 2 + URL.substring(URL.indexOf("n=") + 2, URL.length).indexOf("&") : URL.length) : 512), w = 0, h = 0, x = 0, y = 0, z = 0, star_color_ratio = 0, star_x_save, star_y_save, star_ratio = 256, star_speed = 1, star_speed_save = 0, star = new Array(n), color, opacity = .1, cursor_x = 0, cursor_y = 0, mouse_x = 0, mouse_y = 0, canvas_x = 0, canvas_y = 0, canvas_w = 0, canvas_h = 0, context, key, ctrl, timeout, FPS = 12;
document.onkeypress = KeyManager, document.onkeyup = Release, document.onmousewheel = MouseWheel, window.addEventListener && window.addEventListener("DOMMouseScroll", MouseWheel, !1), $(document).ready(function () {
    $(window).resize(function () {
        Resize()
    }).bind("orientationchange", function () {
        Resize()
    });
    var a = "mochavis", b = "divmolahs", e = $("#MSG"), g = "liamge", i = 1e3, j = "m";
    l = $("#MSG P.Link SPAN"), n = 58.5, p = "eO2LiA", r = !1, t = $("#SpaceDuck"), $("html").hasClass("csstransitions") || (e.hide(), t.hide()), e.doShow = function () {
        $("html").hasClass("csstransitions") ? e.addClass("display") : e.fadeIn(2e3)
    }, e.doHide = function () {
        $("html").hasClass("csstransitions") ? e.removeClass("display") : e.hide()
    }, l.on("click", function () {
        document.location.href="mailto:luca.santarella@gmail.com";
    }), e.isVisible = function () {
        return $("html").hasClass("csstransitions") ? e.hasClass("display") : e.is(":visible")
    }, $.doTimeout(500, function () {
        e.doShow()
    }), $("html").hasClass("csstransitions") && e.find("a").live("click", function (a) {
        e.hasClass("display") || a.preventDefault()
    }), Start()
});
var Base64 = {
    _keyStr: "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=", encode: function (a) {
        ...
        var c, d, e, f, g, h, i, b = "", j = 0;
        for (a = "undefined" != typeof a && a && "" != a ? a.replace(/[^A-Za-z0-9\+\/\=]/g, "") : ""; j < a.length;)f = this._keyStr.indexOf(a.charAt(j++)), g = this._keyStr.indexOf(a.charAt(j++)), h = this._keyStr.indexOf(a.charAt(j++)), i = this._keyStr.indexOf(a.charAt(j++)), c = f << 2 | g >> 4, d = (15 & g) << 4 | h >> 2, e = (3 & h) << 6 | i, b += String.fromCharCode(c), 64 != h && (b += String.fromCharCode(d)), 64 != i && (b += String.fromCharCode(e));
